...
PROMPT_STRUCTURATION_MD: str = (
    ...
    "confiées à l'expert. ATTENTION à la distinction :\n"
    "   - L'OBJET DE MISSION est le chapeau introductif (ex: « Réaliser une expertise "
    "psychologique sur... »). C'est UNE phrase générale. NE PAS la numéroter comme question.\n"
    "   - Les QUESTIONS sont les points DÉTAILLÉS que l'expert doit traiter. Elles sont "
    "souvent introduites par des tirets (-), des lettres (a. b. c.) ou des numéros "
    "(1° 2° 3° ou 1) 2) 3)) dans le texte original.\n"
    "   Présente-les dans une section dédiée au format STRICT suivant :\n"
    "   ```\n"
    "   ## Questions du Tribunal\n\n"
    "   Objet de mission : Réaliser une expertise psychologique...\n\n"
    "   1. Texte exact de la première question\n"
    "   2. Texte exact de la deuxième question\n"
    "   3. Texte exact de la troisième question\n"
    "   ```\n"
    "   RÈGLES STRICTES pour les questions :\n"
    "   - Chaque question sur UNE SEULE ligne, numérotée 1. 2. 3. etc.\n"
    "   - Si le texte original utilise des tirets (-) ou lettres (a. b.) pour lister "
    "les points, convertis-les en numérotation 1. 2. 3.\n"
    "   - L'objet de mission est UNIQUEMENT la première phrase introductive AVANT "
    "les points numérotés (ex: « Réaliser une expertise psychologique »). "
    "NE PAS fusionner les questions dans l'objet de mission.\n"
    "   - NE PAS reformuler ni résumer les questions. Copie-les TELLES QUELLES "
    "depuis le texte original.\n"
    "   - Ne fusionne PAS plusieurs questions en une seule.\n"
    ...
    "5. Corrige UNIQUEMENT les erreurs évidentes d'OCR (caractères mal reconnus comme 'l' → 'I', "
    "mots coupés par un retour à la ligne). NE MODIFIE PAS la conjugaison, l'orthographe "
    "des mots correctement écrits, ni la grammaire du texte original. Si un mot est correct "
    "dans l'original, conserve-le tel quel.\n"
...
PROMPT_CHATBOT: str = (
    ...
    "documentation du système (guide-utilisateur, mentions légales, CGU) :\n\n"
    "{contexte_rag}\n\n"
    "Consignes :\n"
    ...
    "la documentation système (guide-utilisateur).\n"
...
class LLMService:
    ...
    async def _preload_model(self, client: httpx.AsyncClient | None = None) -> None:
        """Force le chargement du modèle en mémoire via un generate."""
        model = ActiveProfile.get_model()
        logger.info("Préchargement du modèle %s...", model)
        try:
            async with httpx.AsyncClient(timeout=180.0) as preload_client:
                resp = await preload_client.post(
                ...
                if resp.status_code == 200:
                    ...
                    logger.warning("Préchargement retourné HTTP %s — %s", resp.status_code, resp.text[:100])
        except Exception as exc:
            logger.warning("Échec du préchargement : %s", exc)
    ...
    async def chat(
        ...
        try:
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                ...
                # Si 404 : le modèle n'est pas chargé en mémoire, forcer le chargement
                if response.status_code == 404:
                    logger.warning("Modèle non chargé (404), tentative de préchargement...")
                    await self._preload_model(client)
                    response = await client.post(
                        f"{self.base_url}/api/chat",
                        json=payload,
                    )
    ...
    async def chat_stream(
        ...
        try:
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                # Pré-test : vérifier que le modèle est chargé
                test_resp = await client.post(
                    f"{self.base_url}/api/chat",
                    json={"model": ActiveProfile.get_model(), "messages": [{"role": "user", "content": "ping"}], "stream": False},
                    timeout=30.0,
                )
                if test_resp.status_code == 404:
                    logger.warning("Modèle non chargé (404), préchargement...")
