...
async def get_performance_profile(
    ...
    active_profile_resp = ProfileResponse(
        ...
        dac_available=profile.dac_available,
    )
...
# Path to .env file (docker-compose reads from this)
_ENV_FILE_PATH = os.environ.get("ENV_FILE_PATH", "/app/.env.docker")
# Fallback: try the mounted .env from docker-compose
if not os.path.exists(_ENV_FILE_PATH):
    _ENV_FILE_PATH = os.environ.get("DOCKER_ENV_PATH", ".env")


_MEMORY_LIMIT_KEYS = {
    "llm": "LLM_MEM_LIMIT",
    "rag": "RAG_MEM_LIMIT",
    "backend": "BACKEND_MEM_LIMIT",
...
def _parse_mem_value_to_mb(value: str) -> int:
    """Parse Docker memory limit string (e.g. '512m', '2g') to MB."""
...
def _mb_to_mem_string(mb: int) -> str:
    """Convert MB to Docker memory limit string."""
    if mb >= 1024 and mb % 1024 == 0:
        return f"{mb // 1024}g"
    return f"{mb}m"


def _read_env_memory_limits() -> dict[str, str]:
    """Read current memory limits from environment or .env file."""
    limits = {}
    for key, env_var in _MEMORY_LIMIT_KEYS.items():
        value = os.environ.get(env_var)
        if not value:
            value = _MEMORY_DEFAULTS[key]
        limits[key] = value
    return limits


def _write_env_memory_limits(limits: dict[str, str]) -> None:
    """Write memory limits to the .env file for docker-compose.

    Reads the existing .env, updates only the memory limit variables,
    and writes back. Creates missing variables if needed.
    """
    ...
    for key, value in limits.items():
        env_var = _MEMORY_LIMIT_KEYS[key]
        updates[env_var] = value

    # Update existing lines or track which ones we need to add
    updated_keys = set()
    new_lines = []
    for line in lines:
        stripped = line.strip()
...
class MemoryLimitsResponse(BaseModel):
    """Current memory limits for all containers."""
    ...
    ram_total_mb: int
    ram_available_for_docker_mb: int


class MemoryLimitsUpdateRequest(BaseModel):
    """Request to update memory limits."""

    llm: str = Field(..., description="LLM container limit (e.g. '8g', '4096m')")
    rag: str = Field(..., description="RAG container limit (e.g. '1g', '512m')")
    backend: str = Field(..., description="Backend container limit (e.g. '512m')")
    frontend: str = Field(..., description="Frontend container limit (e.g. '512m')")
    ocr: str = Field(..., description="OCR container limit (e.g. '512m')")


# Overhead for OS + user apps (browser, Office) in MB
_SYSTEM_OVERHEAD_MB = 7 * 1024  # 7 Go


@router.get("/memory-limits", response_model=MemoryLimitsResponse)
async def get_memory_limits(
    _user: dict = Depends(get_current_user),
):
    """Get current Docker container memory limits."""
    import psutil

    limits = _read_env_memory_limits()
    total_mb = sum(_parse_mem_value_to_mb(v) for v in limits.values())
    ram_total_mb = int(psutil.virtual_memory().total / (1024 * 1024))
    ram_available = max(0, ram_total_mb - _SYSTEM_OVERHEAD_MB)
    ...
    return MemoryLimitsResponse(
        ...
        ram_available_for_docker_mb=ram_available,
    )


@router.put("/memory-limits")
async def update_memory_limits(
    body: MemoryLimitsUpdateRequest,
    _user: dict = Depends(get_current_user),
):
    """Update Docker container memory limits.

    Validates that total allocation does not exceed available RAM,
    and that each limit meets the minimum requirement.
    Writes to .env file. Requires restart to take effect.
    """
    import psutil
    ...
    # Validate format and minimums
    for key, value in new_limits.items():
        try:
            mb = _parse_mem_value_to_mb(value)
        except (ValueError, IndexError):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"Format invalide pour '{key}': '{value}'. Utilisez '512m' ou '2g'.",
            )
        min_mb = _MEMORY_MINIMUMS_MB[key]
        if mb < min_mb:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"'{key}' doit être au minimum {min_mb} Mo (reçu: {mb} Mo).",
            )

    # Validate total does not exceed available RAM
    total_mb = sum(_parse_mem_value_to_mb(v) for v in new_limits.values())
    ram_total_mb = int(psutil.virtual_memory().total / (1024 * 1024))
    ram_available = ram_total_mb - _SYSTEM_OVERHEAD_MB

    if total_mb > ram_available:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=(
                f"Total alloué ({total_mb} Mo) dépasse la RAM disponible pour Docker "
                f"({ram_available} Mo = {ram_total_mb} Mo total − {_SYSTEM_OVERHEAD_MB} Mo réservés OS/utilisateur). "
                f"Réduisez les limites."
            ),
        )

    # Write to .env
    _write_env_memory_limits(new_limits)

    return {
        "status": "ok",
        "message": "Limites mémoire mises à jour. Redémarrage nécessaire pour appliquer.",
        "total_allocated_mb": total_mb,
...
async def restart_containers(
    ...
    """Trigger a graceful restart of all Docker containers.

    Uses docker compose restart via subprocess. The backend container
    itself will restart, so the client should expect a brief disconnection.
    """
    import subprocess
    ...
    try:
        ...
        # Restart with up -d (will re-read .env with new mem limits)
        result = subprocess.run(
            ["docker", "compose", "up", "-d"],
            cwd=compose_dir,
            capture_output=True,
        ...
        if result.returncode != 0:
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=f"Erreur lors du redémarrage: {result.stderr[:200]}",
            )

        return {"status": "ok", "message": "Redémarrage en cours…"}

    except subprocess.TimeoutExpired:
        return {"status": "ok", "message": "Redémarrage initié (timeout — vérifiez le status)."}
    except FileNotFoundError:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Docker Compose non trouvé sur le système.",
        )
