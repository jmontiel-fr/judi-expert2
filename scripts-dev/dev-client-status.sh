#!/bin/bash
# Usage : bash scripts-dev/dev-client-status.sh

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$SCRIPT_DIR/_common.sh"
parse_args "dev-client-status.sh" \
  "Afficher le statut de tous les composants du Site Client" \
  "" "" "$@"

echo -e "${BLUE}══════════════════════════════════════════════${NC}"
echo -e "${BLUE}  Judi-Expert Client — Statut${NC}"
echo -e "${BLUE}══════════════════════════════════════════════${NC}"
echo ""
...
if docker info > /dev/null 2>&1; then
  ...
  echo ""
  echo -e "  Lancez : bash scripts-dev/dev-client-start.sh"
  exit 1
fi
echo ""
