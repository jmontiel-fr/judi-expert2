#!/bin/bash
set -e
# Usage : bash scripts-dev/dev-client-restart.sh [--build] [--pull-llm]

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"
COMPOSE="$ROOT_DIR/client-site/docker-compose.yml"
source "$SCRIPT_DIR/_common.sh"
parse_args "dev-client-restart.sh" \
  "Redémarrer le Site Client (arrêt + libération ports + démarrage)" \
  "yes" "yes" "$@"
...
echo -e "${BLUE}══════════════════════════════════════════════${NC}"
echo -e "${BLUE}  Judi-Expert Client — Redémarrage${NC}"
echo -e "${BLUE}══════════════════════════════════════════════${NC}"
echo ""
...
echo -e "${GREEN}  ✔ Site Client redémarré${NC}"
