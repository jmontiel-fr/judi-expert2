...
async def get_current_expert(
    ...
    if IS_DEV and access_token.startswith("dev-token-"):
        # Mode dev : retrouver l'expert via le token stocké en session côté frontend
        # On cherche le dernier expert inscrit (simplifié pour le dev)
        # Le frontend envoie l'email dans un header custom X-Dev-Email
