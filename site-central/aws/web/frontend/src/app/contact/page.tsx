...
interface FormData { domaine: string; objet: string; message: string; }
interface FormErrors { domaine?: string; objet?: string; message?: string; }

export default function ContactPage() {
  const { accessToken } = useAuth();
  const [formData, setFormData] = useState<FormData>({ domaine: "", objet: "", message: "" });
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      await apiSubmitContact(
        { domaine: formData.domaine, objet: formData.objet, message: formData.message.trim() },
        accessToken,
      );
      setSubmitSuccess("Votre message a été envoyé avec succès. Nous vous répondrons dans les meilleurs délais.");
      setFormData({ domaine: "", objet: "", message: "" });
    } catch {
