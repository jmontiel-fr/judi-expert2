...
```
judi-expert/
├── client-site/                        # Site Client (expert's PC)
...
│   ├── app_client_package/             # Installer packaging (NSIS for Windows, shell for Unix)
│   └── web/
...
│   ├── dev-client-start.sh             # Start Site Client (--build, --pull-llm)
│   ├── dev-client-stop.sh              # Stop Site Client
│   ├── dev-client-restart.sh           # Restart Site Client
│   ├── dev-client-status.sh            # Status of client containers
...
- **Mirrored structure**: `client-site/web/` and `central-site/web/` share the same layout (backend with models/routers/services, frontend with src/app/components)
...
- **Client backend adds `middleware/`**: Data isolation middleware for multi-tenant safety
...
- `{app}` : `client` or `central`
- `{version}` : semantic versioning `x.y.z` (e.g. `1.0.0`, `1.2.3`)
- `{ext}` : file extension (`.tar.gz`, `.exe`, `.zip`)

**Examples:**
- `judi-expert-client-1.0.0.tar.gz` — Docker images archive for client app
- `judi-expert-client-1.0.0.exe` — Windows installer for client app
...
├── packages/
│   ├── client/
│   │   ├── judi-expert-client-1.0.0.exe
│   │   ├── judi-expert-client-1.0.0.tar.gz
│   │   └── latest.json                    # {"version": "1.0.0", "url": "..."}
│   └── central/
│       └── judi-expert-central-1.0.0.tar.gz
└── images/
    ├── judi-expert-client-backend-1.0.0.tar.gz
    ├── judi-expert-client-ocr-1.0.0.tar.gz
    └── judi-expert-client-frontend-1.0.0.tar.gz
```
