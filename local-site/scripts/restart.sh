...
# ─────────────────────────────────────────────────────────
# restart.sh — Full restart + rebuild of all Judi-Expert
#              local containers (down + build + up)
# Exigences : 30.1
# ─────────────────────────────────────────────────────────
...
free_port() {
  local port=$1
  # Find Docker containers bound to this port and stop them
  local cid
  ...
  # Fallback: check for non-Docker processes
  local pid
  ...
  case "$pname" in
    com.docker.*|docker.exe|Docker\ Desktop.exe|dockerd.exe)
      # Docker proxy — will be freed by compose down
      return
      ;;
  esac
...
echo -e "${BLUE}══════════════════════════════════════════════${NC}"
echo -e "${BLUE}  Judi-Expert Local — Redémarrage complet${NC}"
echo -e "${BLUE}══════════════════════════════════════════════${NC}"
echo ""
...
# ── Arrêt du stack ────────────────────────────────────────
echo -e "${YELLOW}[1/3] Arrêt des services...${NC}"
...
echo -e "${YELLOW}[2/3] Vérification des ports...${NC}"
...
# ── Build + démarrage ────────────────────────────────────
echo -e "${YELLOW}[3/3] Build et démarrage...${NC}"
